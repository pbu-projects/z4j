package lol.pbu.z4j.model;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import io.micronaut.core.annotation.Nullable;
import io.micronaut.serde.annotation.Serdeable;
import jakarta.annotation.Generated;
import jakarta.validation.Valid;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;
import lombok.experimental.Accessors;

import java.util.ArrayList;
import java.util.List;

/**
 * TicketsCreateRequest
 */
@Accessors(chain = true)
@NoArgsConstructor
@AllArgsConstructor
@Data
@JsonPropertyOrder(TicketsCreateRequest.JSON_PROPERTY_TICKETS)
@Serdeable
@Generated("io.micronaut.openapi.generator.JavaMicronautClientCodegen")
public class TicketsCreateRequest {

    public static final String JSON_PROPERTY_TICKETS = "tickets";

    @Nullable
    @JsonProperty(JSON_PROPERTY_TICKETS)
    @JsonInclude(JsonInclude.Include.USE_DEFAULTS)
    private List<@Valid TicketCreateInput> tickets;

    /**
     * Add an item to the tickets property in a chainable fashion.
     *
     * @return The same instance of TicketsCreateRequest for chaining.
     */
    public TicketsCreateRequest addTicketsItem(TicketCreateInput ticketsItem) {
        if (tickets == null) {
            tickets = new ArrayList<>();
        }
        tickets.add(ticketsItem);
        return this;
    }

}